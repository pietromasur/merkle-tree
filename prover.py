...
def gen_merkle_proof(leaves, pos):
    ...
    level_state = state # local copy of state (minha modif)
    for level in range(height):
        print(level,level_pos, len(level_state))
        if level_pos%2==0:
            # Consider pos as left on hash
            path.append(level_state[level_pos+1])
            # Getting new position
            level_pos = int(level_pos/2)
        else:
            # Consider pos as right on hash
            path.append(level_state[level_pos-1])
            level_pos = int((level_pos-1)/2)
        # Compute new level of merkle tree
        new_level_state = []
        for i, (previous, current) in enumerate(zip(level_state, level_state[1:])):
            if i%2==0:
                new_level_state.append(hash_internal_node(previous, current))
